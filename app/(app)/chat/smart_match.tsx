...
export default function SmartMatchResultsScreen() {
  ...
  const [matches, setMatches] = useState<(MatchResult & { profile: UserProfile })[]>([]);
  ...
  const runMatching = async () => {
    ...
    const allUids = response.matches.flatMap((m) => m.userIds);
    const profiles = await getUsersByIds(allUids);
    const profileMap: Record<string, UserProfile> = {};
    profiles.forEach((p) => { profileMap[p.uid] = p; });

    const enriched = response.matches
      .filter((m) => profileMap[m.userIds[0]])
      .map((m) => ({ ...m, profile: profileMap[m.userIds[0]] }));
    setMatches(enriched);
    setLoading(false);
  };

  useEffect(() => { runMatching(); }, [user?.uid]);
    ...
    <SafeAreaView className="flex-1 bg-white">
      ...
      {loading ? (
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="text-[15px] text-[#8E8E93] mt-3" style={{ fontFamily: 'PlusJakartaSans-Regular' }}>Finding matches...</Text>
        </View>
      ) : (
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          <View className="items-center justify-center pt-8 pb-4">
            ...
            <Text className="text-[20px] font-bold text-black" style={{ fontFamily: 'PlusJakartaSans-Bold' }}>
              {matches.length > 0 ? `We found ${matches.length} great matches for you!` : 'No matches found'}
            </Text>
          </View>

          <View className="px-5 pt-2">
            {matches.map((match) => (
              <View key={match.userIds[0]} className="bg-white border border-[#E5E5EA] rounded-xl p-4 mb-4 shadow-sm">
                <View className="flex-row items-center justify-between mb-3">
                  <View className="bg-[#F2F2F7] px-3 py-1 rounded-full">
                    <Text className="text-[12px] text-[#4A4A4A] font-medium" style={{ fontFamily: 'PlusJakartaSans-Medium' }}>
                      {match.profile.role === 'alumni' ? 'Alumni' : 'Student'}
                    </Text>
                  </View>
                ...
                <View className="flex-row items-center mb-4">
                  <Image source={match.profile.profilePhoto ? { uri: match.profile.profilePhoto } : DEFAULT_AVATAR} className="w-14 h-14 rounded-full bg-gray-200" />
                  <View className="flex-1 ml-4 justify-center">
                    <Text className="text-[18px] font-bold text-black" style={{ fontFamily: 'PlusJakartaSans-Bold' }}>{match.profile.displayName}</Text>
                    <Text className="text-[14px] text-[#8E8E93] mt-0.5" style={{ fontFamily: 'PlusJakartaSans-Regular' }}>{match.profile.programme || match.profile.role}</Text>
                  </View>
                </View>
                ...
                <View className="flex-row gap-3">
                  ...
                  <TouchableOpacity className="flex-1 border border-[#1B1C62] py-3 rounded-xl items-center justify-center"
                    onPress={() => router.push(`/profile/view/${match.profile.uid}` as any)}>
          ...
          <TouchableOpacity className="items-center py-4 mb-4" onPress={runMatching}>
