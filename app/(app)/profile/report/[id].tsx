  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      <View className="flex-row items-center justify-between px-4 py-3 bg-white border-b border-[#E5E5EA]">
        ...
        <Text className="text-[18px] font-bold text-black" style={{ fontFamily: 'PlusJakartaSans-Bold' }}>Report User</Text>
        <TouchableOpacity className="w-16 h-10 items-center justify-center -mr-2" disabled={!selectedReason || submitting} onPress={handleSubmitReport}>
          {submitting ? <ActivityIndicator size="small" color="#D71440" /> : (
            <Text className={`text-[16px] font-bold ${selectedReason ? 'text-[#D71440]' : 'text-gray-300'}`} style={{ fontFamily: 'PlusJakartaSans-Bold' }}>Submit</Text>
          )}
        </TouchableOpacity>
      </View>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
          <View className="px-5 pt-6 pb-2">
            <Text className="text-[20px] font-bold text-black mb-2" style={{ fontFamily: 'PlusJakartaSans-Bold' }}>Why are you reporting this user?</Text>
            <Text className="text-[15px] text-[#666666] leading-6 mb-6" style={{ fontFamily: 'PlusJakartaSans-Regular' }}>
              Your report is anonymous, except if you're reporting an intellectual property infringement. The user will not be notified that you reported them.
            </Text>

            <Text className="text-[14px] text-[#8E8E93] uppercase mb-2 font-medium" style={{ fontFamily: 'PlusJakartaSans-Medium' }}>Select a Reason</Text>
            ...
            {selectedReason && (
              <View>
                <Text className="text-[14px] text-[#8E8E93] uppercase mb-2 font-medium" style={{ fontFamily: 'PlusJakartaSans-Medium' }}>Additional Details (Optional)</Text>
                <View className="bg-white rounded-xl border border-[#E5E5EA] p-4 mb-8 h-32">
                  <TextInput className="flex-1 text-[16px] text-black" style={{ fontFamily: 'PlusJakartaSans-Regular' }} placeholder="Provide more context to help us understand..." placeholderTextColor="#C7C7CC" multiline textAlignVertical="top" value={additionalDetails} onChangeText={setAdditionalDetails} />
                </View>
              </View>
            )}
